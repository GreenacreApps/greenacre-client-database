...
async function deleteAssetField(req: Request, fieldId: string): Promise<Response> {
  const user = await requireAdmin(req);
  if (user instanceof Response) return user;
  if (!fieldId) return text("Data cell id required", 400);
  const fields = await store().get(ASSET_FIELDS_KEY, { type: "json" }) || [];
  const nextFields = fields.filter((field: { id: string }) => field.id !== fieldId);
  await store().setJSON(ASSET_FIELDS_KEY, nextFields);

  const data = await store().get(DATA_KEY, { type: "json" }) || { version: 0, state: {}, assetCustomFields: [] };
  const clients = Array.isArray(data.state?.clients) ? data.state.clients : [];
  for (const client of clients) {
    for (const site of client.sites || []) {
      for (const asset of site.assets || []) {
        if (asset.customData) delete asset.customData[fieldId];
      }
    }
  }
  data.assetCustomFields = nextFields;
  data.version = Number(data.version || 0) + 1;
  data.updatedAt = new Date().toISOString();
  data.updatedBy = user.username;
  await store().setJSON(DATA_KEY, data);
...
export default async (req: Request, context: Context): Promise<Response> => {
  ...
  if (req.method === "DELETE" && path.startsWith("/api/asset-fields/")) return deleteAssetField(req, decodeURIComponent(path.slice("/api/asset-fields/".length)));
