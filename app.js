...
const defaultState = {
  ...
  clients: [
    {
      id: "test-client-001",
      name: "Test Water Services",
      address: "1 Test House, Example Road, Doncaster, DN1 1AA",
      contacts: [
        { id: "test-contact-001", name: "Alex Test", role: "Project Engineer", email: "alex.test@example.com", phone: "07123 456789" },
        { id: "test-contact-002", name: "Sam Sample", role: "Commercial Contact", email: "sam.sample@example.com", phone: "07987 654321" },
        { id: "test-contact-003", name: "Jordan Trial", role: "Operations Contact", email: "jordan.trial@example.com", phone: "07700 900123" }
      ],
      sites: [
        {
          id: "test-site-001",
          reference: "TEST-STW-001",
          name: "Test Sewage Treatment Works",
          address: "Test Lane, Sampletown, DN2 2BB",
          assets: [
            { id: "test-asset-001", reference: "ASSET-ODOUR-001", name: "Inlet Works Odour Control", location: "Inlet works" },
            { id: "test-asset-002", reference: "ASSET-TANK-002", name: "Sludge Tank Ventilation", location: "Sludge tanks" }
          ]
        },
        {
          id: "test-site-002",
          reference: "TEST-PS-002",
          name: "Test Pumping Station",
          address: "Pump House Road, Sampletown, DN3 3CC",
          assets: [
            { id: "test-asset-003", reference: "ASSET-PS-003", name: "Pumping Station Odour Control", location: "Wet well" }
          ]
        }
      ]
    }
  ...
  enquiries: [],
  quotes: [],
  activeClientId: "test-client-001",
  activeEnquiryId: "",
  activeQuoteId: ""
};
...
async function renderAdminUsers() {
  ...
  try {
    ...
    list.innerHTML = `<h3>Users</h3>${(data.users || []).map((user) => `<p><strong>${escapeHtml(user.username)}</strong> - ${escapeHtml(user.role)}</p>`).join("")}`;
  } catch (error) {
...
function findOrCreateSite(client, reference, name, address = "") {
  ...
  if (!site) {
    site = { id: newId("site"), reference: safeRef, name: safeName, address: address || "", assets: [] };
    client.sites ||= [];
...
function exportClientDatabaseExcel() {
  if (!window.XLSX) {
    alert("Excel export could not start because the Excel library has not loaded. Refresh the page and try again.");
    return;
  }
  ...
  XLSX.writeFile(workbook, `greenacre-client-database-${todayIso()}.xlsx`);
}

async function importClientDatabaseExcel(file) {
  if (!window.XLSX) throw new Error("Excel import could not start because the Excel library has not loaded. Refresh the page and try again.");
...
async function loadStateFromServer() {
  ...
  return normalizeLockedLogic(mergeDefaults(clone(defaultState), data.state || {}));
}
...
function renderCustomAssetFields(asset = {}) {
    ...
    <div class="custom-field-list">
      ${fields.map((field) => `
        <label>${escapeHtml(field.label)}
          <input data-custom-asset-field="${escapeHtml(field.id)}" type="${field.type === "number" ? "number" : field.type === "date" ? "date" : "text"}" value="${escapeHtml(normalizedAsset.customData?.[field.id] ?? "")}">
        </label>
      `).join("")}
    </div>
  ` : "";
  ...
  if (adminPanel) {
    adminPanel.hidden = !isAdminUser();
    const existing = adminPanel.querySelector(".asset-field-admin-list");
    if (existing) existing.remove();
    if (isAdminUser() && fields.length) {
      const list = document.createElement("div");
      list.className = "asset-field-admin-list record-list";
      list.innerHTML = fields.map((field) => `
        <article class="contact-row">
          <strong>${escapeHtml(field.label)}</strong>
          <span>${escapeHtml(field.type || "text")}</span>
          ${adminDeleteButton(`data-remove-asset-field="${escapeHtml(field.id)}"`)}
        </article>
      `).join("");
...
function updateCustomAssetField(target) {
  ...
  if (!asset) return;
  asset.customData ||= {};
  asset.customData[target.dataset.customAssetField] = target.type === "number" ? n(target.value) : target.value;
}
...
async function createAssetCustomField() {
  ...
  try {
    const data = await apiFetch(SERVER_STORAGE.assetFieldsUrl, {
      method: "POST",
      body: JSON.stringify({ label, type })
    });
    assetCustomFields = data.assetCustomFields || [];
    if ($("#newAssetFieldLabel")) $("#newAssetFieldLabel").value = "";
    if (message) message.textContent = "Data cell added for all assets.";
    ...
    if (message) message.textContent = error.message || "Could not add data cell.";
  }
}
...
async function loadStateFromSharePoint() {
  ...
  (assetData?.value || []).forEach((item) => {
    ...
    site.assets.push(normalizeAsset({
      ...
      carbonHighHighSetpoint: numberFieldValue(item, assetFields, "CarbonHighHighSetpoint"),
...
function loadStateFromLocalStorage(error) {
  try {
    ...
    setSyncStatus(
      ...
      "error",
      error ? `${error.message || error}. SharePoint unavailable; changes are saved only in this browser.` : "Changes are saved only in this browser."
    );
...
function saveStateToLocalStorage() {
  try {
    ...
    if (!sharePointLoaded) {
      setSyncStatus("Saved locally only", "error", "This file is not running from a SharePoint page, so changes are saved only in this browser.");
    }
  } catch (error) {
    console.error(error);
      ...
      for (const site of client.sites || []) {
        ...
        for (const assetRaw of site.assets || []) {
          ...
          await upsertSharePointItem("assets", asset.id, {
            ...
            ...optionalNumberBody(sharePointFields.assets, "CarbonHighHighSetpoint", asset.carbonHighHighSetpoint),
...
function renderClients() {
  ...
  setBlock("#clientRegisterPanel", !clientEditorOpen);
  setBlock("#clientEditor", clientEditorOpen);
  ...
  if (!activeClientSiteId || !client.sites.some((site) => site.id === activeClientSiteId)) {
    activeClientSiteId = client.sites[0]?.id || "";
  }
  ...
  if (!activeClientAssetId || !site.assets?.some((asset) => asset.id === activeClientAssetId)) {
    activeClientAssetId = site.assets?.[0]?.id || "";
  }
  ...
  $("#clientSiteRef").value = site.reference || "";
  $("#clientSiteName").value = site.name || "";
  $("#clientSiteAddress").value = site.address || "";
  ...
  setBlock("#clientDetailsPanel", clientDrillLevel === "client-details");
  setBlock("#clientSitesPanel", clientDrillLevel === "client");
  setBlock("#clientAssetRegisterPanel", clientDrillLevel === "site");
  setBlock("#clientAssetDetailPanel", clientDrillLevel === "asset");
}
      ...
      <td colspan="8">
        <div class="asset-detail-panel">
          ...
          <div class="form-grid compact">
            ...
            ${field("Flow rate m3/h", "flowRate", "number")}
            ...
            ${field("Carbon media volume m3", "carbonMediaVolume", "number")}
            ${field("Carbon PT pressure drop", "carbonPressureDrop", "number")}
            ${field("Carbon PT high setpoint", "carbonHighSetpoint", "number")}
            ${field("Carbon PT high-high setpoint", "carbonHighHighSetpoint", "number")}
            ...
            ${field("Biofilter media volume m3", "bioMediaVolume", "number")}
          </div>
        </div>
      </td>
    </tr>
...
function renderClientSites() {
  ...
  `).join("") : `<tr><td colspan="4">No sites added yet.</td></tr>`;
}
...
function bindDynamicTableEvents() {
  ...
  document.body.addEventListener("click", (event) => {
    ...
    if (removeAssetFieldId !== undefined) {
      if (!requireAdminRemoval()) return;
      removeAssetCustomField(removeAssetFieldId);
    }
...
function updateClientDraftFromForm() {
  if (!clientDraft || !$("#clientName")) return;
  clientDraft.name = $("#clientName").value.trim();
  clientDraft.address = $("#clientAddress").value.trim();
  clientDraft.contacts ||= [];
  clientDraft.sites ||= [];
  let site = clientDraft.sites.find((item) => item.id === activeClientSiteId);
  const hasSiteValues = ["clientSiteRef", "clientSiteName", "clientSiteAddress"].some((id) => $(`#${id}`).value.trim());
  if (!site && hasSiteValues) {
    site = blankSite(clientDraft.sites.length + 1);
  ...
  const hasAssetValues = assetFieldIds.some((id) => String($(`#${id}`).value || "").trim() && Number($(`#${id}`).value) !== 0);
  ...
  Object.assign(asset, {
    ...
    flowRate: n($("#assetFlowRate").value),
    ...
    flowSetpointPercent: n($("#assetFlowSetpoint").value),
    carbonMediaType: $("#assetCarbonMediaType").value.trim(),
    carbonMediaSlNumber: $("#assetCarbonMediaSl").value.trim(),
    carbonMediaVolume: n($("#assetCarbonMediaVolume").value),
    carbonPressureDrop: n($("#assetCarbonPressureDrop").value),
    carbonHighSetpoint: n($("#assetCarbonHighSetpoint").value),
    carbonHighHighSetpoint: n($("#assetCarbonHighHighSetpoint").value),
    ...
    bioMediaVolume: n($("#assetBioMediaVolume").value)
  });
  asset.customData ||= {};
  (assetCustomFields || []).forEach((field) => {
    const input = document.querySelector(`[data-custom-asset-field="${CSS.escape(field.id)}"]`);
    if (input) asset.customData[field.id] = input.type === "number" ? n(input.value) : input.value;
  });
  renderAssetRegister();
}
...
function bindActions() {
  ...
  $("#addSiteBtn").addEventListener("click", () => {
    ...
    updateClientDraftFromForm();
    const site = blankSite((clientDraft.sites || []).length + 1);
    clientDraft.sites ||= [];
    clientDraft.sites.push(site);
    activeClientSiteId = site.id;
    activeClientAssetId = "";
  ...
  $("#addAssetBtn").addEventListener("click", () => {
    ...
    site.assets ||= [];
    const asset = blankAsset(site.assets.length + 1);
    site.assets.push(asset);
    activeClientAssetId = asset.id;
    clientDrillLevel = "asset";
    update();
  ...
  [
    ...
    $(`#${id}`).addEventListener("input", updateClientDraftFromForm);
    $(`#${id}`).addEventListener("change", updateClientDraftFromForm);
  });
  document.addEventListener("input", (event) => {
    if (event.target?.dataset?.customAssetField) updateCustomAssetField(event.target);
  });
  document.addEventListener("change", (event) => {
    if (event.target?.dataset?.customAssetField) updateCustomAssetField(event.target);
  });
  $("#addAssetFieldBtn")?.addEventListener("click", createAssetCustomField);
  ...
  $("#importInput").addEventListener("change", async (event) => {
    ...
    if (!file) return;
    try {
      await importClientDatabaseExcel(file);
    } catch (error) {
